...
class MyScene(Scene):
    ...
    def _get_misc(self):
        ...
        # Add segmentation info for all objects
        object_informations = {}
        # Make sure that we have the objects in the scene
        for obj in self.pyrep.get_objects_in_tree():
            handle = obj.get_handle()
            # The segmentation color is derived from the object's handle.
            # R = handle % 256, G = (handle // 256) % 256, B = (handle // (256*256)) % 256
            obj_info = {
                'handle': handle,
                'segmentation_color': [ (handle % 256) / 255.0, ((handle // 256) % 256) / 255.0, ((handle // (256 * 256)) % 256) / 255.0, ]
            }
            object_informations[handle] = obj_info
        misc['object_informations'] = object_informations
        ...
        if self.enable_stage:
            ...
            for stage_idx in range(stage_num):
                ...
                grasp_obj = Shape(grasp_obj_name)
                misc.update({f'stage{stage_idx}_grasp_obj_name': grasp_obj_name})
                misc.update({f'stage{stage_idx}_grasp_obj_handle': grasp_obj.get_handle()})
                grasp_obj_pose = grasp_obj.get_pose()
                ...
                target_obj = Shape(target_obj_name)
                misc.update({f'stage{stage_idx}_target_obj_name': target_obj_name})
                misc.update({f'stage{stage_idx}_target_obj_handle': target_obj.get_handle()})
                target_obj_pose = target_obj.get_pose()
            ...
            misc.update({f'grasp_obj_handle': grasp_obj.get_handle()})
            grasp_obj_pose = grasp_obj.get_pose()
            ...
            misc.update({f'target_obj_handle': target_obj.get_handle()})
            target_obj_pose = target_obj.get_pose()
