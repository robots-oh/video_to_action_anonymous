...
class SimpleDP3(BasePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], target=None) -> Dict[str, torch.Tensor]:
        ...
        local_cond = None
        global_cond = None
        # simple_dp3.py 파일의 predict_action 메소드 내부
# if self.obs_as_global_cond: 블록부터 그에 해당하는 else 블록까지를 아래 내용으로 교체

        if self.obs_as_global_cond:
            # 1. 이미지 텐서는 모양을 바꾸면 안 되므로 잠시 분리합니다.
            nobs_img = nobs.pop('grasp_image', None)

            # 2. 이미지를 제외한 나머지 데이터에만 기존의 reshape 로직을 적용합니다.
            this_nobs = dict_apply(nobs, 
                lambda x: x[:,:To,...].reshape(-1,*x.shape[2:]))

            # 3. 분리해두었던 이미지 텐서를 올바른 형태로 다시 딕셔너리에 추가합니다.
            if nobs_img is not None:
                if len(nobs_img.shape) == 5: # (B, T, C, H, W) 형태일 경우
                    this_nobs['grasp_image'] = nobs_img[:,:To,...].reshape(-1, *nobs_img.shape[2:])
                elif len(nobs_img.shape) == 4: # 이미 (B, C, H, W) 형태일 경우
                    this_nobs['grasp_image'] = nobs_img
            ...
            # else 구문도 동일한 원리로 수정합니다.
            # 1. 이미지 텐서를 nobs 딕셔너리에서 잠시 분리합니다.
            nobs_img = nobs.pop('grasp_image', None)

            # 2. 이미지를 제외한 나머지 데이터에만 기존의 reshape 로직을 적용합니다.
            this_nobs = dict_apply(nobs, lambda x: x[:,:To,...].reshape(-1, *x.shape[2:]))
            
            # 3. 분리해두었던 이미지 텐서를 올바른 형태로 다시 넣어줍니다.
            if nobs_img is not None:
                if len(nobs_img.shape) == 5:
                    this_nobs['grasp_image'] = nobs_img[:,:To,...].reshape(-1, *nobs_img.shape[2:])
                elif len(nobs_img.shape) == 4:
                    this_nobs['grasp_image'] = nobs_img
    ...
    def compute_loss(self, batch):
        ...
        #     # 3. 이미지 텐서가 있다면, 올바른 방식으로 모양을 변환하여 다시 추가합니다.
        #     # (Batch, Time, C, H, W) -> (Batch * Time, C, H, W)
        #     if nobs_img is not None:
        #         n_obs_steps = self.n_obs_steps
        #         this_nobs['grasp_image'] = nobs_img[:,:n_obs_steps,...].reshape(-1, *nobs_img.shape[2:])
        ...
        #     # 3. 이미지 텐서가 있다면, 올바른 방식으로 모양을 변환하여 다시 추가합니다.
        #     if nobs_img is not None:
        #         this_nobs['grasp_image'] = nobs_img.reshape(-1, *nobs_img.shape[2:])
        ...
        if self.obs_as_global_cond:
            ...
            # 3. 분리해두었던 원본 이미지 텐서를 (모양을 바꾸지 않고) 그대로 다시 넣어줍니다.
            if nobs_img is not None:
                # 이미지 텐서는 이미 올바른 [B, C, H, W] 형태이므로 reshape이 필요 없습니다.
                # 단, n_obs_steps > 1인 경우를 대비해 n_obs_steps 만큼 batch를 복제해줍니다.
                n_obs_steps = self.n_obs_steps
                if n_obs_steps > 1:
                    nobs_img = nobs_img.unsqueeze(1).repeat(1,n_obs_steps,1,1,1)
                    this_nobs['grasp_image'] = nobs_img.reshape(-1, *nobs_img.shape[2:])
                else:
                    this_nobs['grasp_image'] = nobs_img
            ...
            # 3. 분리해두었던 원본 이미지 텐서를 그대로 다시 넣어줍니다.
            if nobs_img is not None:
                this_nobs['grasp_image'] = nobs_img.reshape(-1, *nobs_img.shape[2:])
