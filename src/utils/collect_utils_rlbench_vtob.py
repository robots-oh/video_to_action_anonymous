...
def _left_is_stopped(demo, i, obs, stopped_buffer, delta=0.1):
...
def _right_is_stopped(demo, i, obs, stopped_buffer, delta=0.1):
    next_is_not_final = i == (len(demo) - 2)
    gripper_state_no_change = (
            i < (len(demo) - 2) and
...
def _left_keypoint_discovery(demo: Demo,
                       stopping_delta=0.1,
                       method='heuristic') -> List[int]:
    episode_keypoints = []
    if method == 'heuristic':
        prev_gripper_open = demo[0].left.gripper_open
        stopped_buffer = 0
        for i, obs in enumerate(demo):
            stopped = _left_is_stopped(demo, i, obs, stopped_buffer, stopping_delta)
...
def _right_keypoint_discovery(demo: Demo,
                       stopping_delta=0.1,
                       method='heuristic') -> List[int]:
    episode_keypoints = []
    if method == 'heuristic':
        ...
        stopped_buffer = 0
        for i, obs in enumerate(demo):
            stopped = _right_is_stopped(demo, i, obs, stopped_buffer, stopping_delta)
            stopped_buffer = 4 if stopped else stopped_buffer - 1
            # If change in gripper, or end of episode.
            last = i == (len(demo) - 1)
...
def keypoint_discovery(demo: Demo,
    ...
    right_episode_keypoints = _right_keypoint_discovery(demo, stopping_delta, method)
    left_episode_keypoints = _left_keypoint_discovery(demo, stopping_delta, method)

    all_keypoints = sorted(list(set(right_episode_keypoints) | set(left_episode_keypoints)))
