...
class Scene(object):
    ...
    def __init__(self,
        ...
        # For bimanual, wrist cameras are named differently.
        if self.robot.is_bimanual:
            self._cam_wrist_left = VisionSensor('cam_wrist_left')
            self._cam_wrist_right = VisionSensor('cam_wrist_right')
            self._cam_wrist_left_mask = VisionSensor('cam_wrist_left_mask')
            self._cam_wrist_right_mask = VisionSensor('cam_wrist_right_mask')
        else:
        ...
        self.camera_sensors = {
            'left_shoulder': self._cam_over_shoulder_left,
            'right_shoulder': self._cam_over_shoulder_right,
            'overhead': self._cam_overhead,
            'front': self._cam_front,
        }
        self.camera_sensors_mask = {
            'left_shoulder': self._cam_over_shoulder_left_mask,
            'right_shoulder': self._cam_over_shoulder_right_mask,
            'overhead': self._cam_overhead_mask,
            'front': self._cam_front_mask,
        }
        if self.robot.is_bimanual:
            self.camera_sensors['wrist_left'] = self._cam_wrist_left
            self.camera_sensors['wrist_right'] = self._cam_wrist_right
            self.camera_sensors_mask['wrist_left'] = self._cam_wrist_left_mask
            self.camera_sensors_mask['wrist_right'] = self._cam_wrist_right_mask
        else:
            self.camera_sensors['wrist'] = self._cam_wrist
            self.camera_sensors_mask['wrist'] = self._cam_wrist_mask

        self._obs_config.camera_configs = {
            name: getattr(self._obs_config, f"{name}_camera") for name in self.camera_sensors.keys()
        }
        ...
        self._workspace_maxz = z + 2.0  # 1M above workspace
