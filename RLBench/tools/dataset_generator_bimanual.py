...
def run_all_variations(task_name, headless, save_path, episodes_per_task, image_size):
    ...
    rlbench_env.launch()
    # process_seed = (os.getpid() + int(np.datetime64('now').astype(int) / 1000)) % (2**32 - 1)
    ...
    for task in tasks:
        ...
        while True:
            # 확인할 에피소드 경로
            episode_path_to_check = os.path.join(episodes_path, EPISODE_FOLDER % start_ex_idx)
            # 해당 에피소드의 완료를 나타내는 파일 (루프의 마지막에 저장되는 파일)
            descriptions_file = os.path.join(episode_path_to_check, VARIATION_DESCRIPTIONS)
            
            if not os.path.exists(descriptions_file):
                # 파일이 존재하지 않으면, 이 인덱스부터 시작합니다.
                # (폴더가 아예 없거나, 생성이 중단된 경우)
                break
            
            # 파일이 존재하면, 이 에피소드는 완료된 것으로 간주하고 다음 인덱스를 확인합니다.
            start_ex_idx += 1
        
        if start_ex_idx > 0:
            logging.info(f"Found {start_ex_idx} existing completed episodes. Resuming from episode {start_ex_idx}.")
        
        if start_ex_idx >= episodes_per_task:
            logging.warning(f"Task {task_name} already has {start_ex_idx} episodes, which meets or exceeds the requested {episodes_per_task}. Skipping.")
            continue  # 이 태스크에 대한 작업은 완료되었으므로 다음 태스크로 넘어갑니다.
        ############################################################################3############################################################################3

        abort_variation = False
        for ex_idx in range(start_ex_idx, episodes_per_task):
            attempts = 10
            while attempts > 0:
                try:
                    ...
                    episode_seed = (os.getpid() + ex_idx + int(np.datetime64('now').astype(int) / 1000)) % (2**32 - 1)
                    np.random.seed(episode_seed)

                    # TODO: for now we do the explicit looping.
                    demo, = task_env.get_demos(amount=1, live_demos=True)
                    print(demo.random_seed)
                    print("==========================")
                    
                #  NoWaypointsError, DemoError,
                except (BoundaryError, WaypointError, InvalidActionError, TaskEnvironmentError, DemoError) as e:
...
@click.command()
@filepath_option("--save_path", default="/data/",  help="Where to save the demos.")
